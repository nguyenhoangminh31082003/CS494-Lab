...
class Button:
	...
	def fillColor(self, gameScreen):
		gameScreen.blit(self.image, (self.rect.x , self.rect.y))
		pygame.draw.rect(gameScreen, Constant.B_COLOR, pygame.Rect(self.coord[0], self.coord[1], self.size[0], self.size[1]))
...
class TextButton(pygame.sprite.Sprite):
	def __init__(self, text, x, y, textSize = 20):
		self.textFont = pygame.font.Font(Constant.AMATICSC_FONT, textSize)
		self.text = text
		self.image = self.textFont.render(self.text, True, Constant.WHITE)
	...
	def collision(self, pos):
		if pos and not self.clicked:
			if self.rect.collidepoint(pos):
				self.image = self.textFont.render(self.text, True, Constant.GREEN)
				self.clicked = True
				return True, self.text
		return False
		
	def draw(self, gameScreen):
		pygame.draw.rect(gameScreen, Constant.WHITE, self.rect, 2)
